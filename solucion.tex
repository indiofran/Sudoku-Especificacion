...
%\integrante{Boncini, Ferando}{004/01}{email4@dominio.com}
...
9. \begin{proc}{sudoku\_esTableroParcialmenteResuelto}{\textbf{\In t}: \matriz{\ent}, \textbf{\Out result}: \bool }{}{}
	...
	\pred{TableroConElementosDelCeroalNueve}{t: \matriz{\ent}}{\\
        (\forall i: \ent)(\forall j: \ent)((enRango(t, i) $\land$ enRango(t[i], j))$\yLuego$
        (0 \leq t[i][j] \leq 9) \\
        }
	...
	TableroConElementosDelCeroalNueve(t)$\land$\\
        (\forall i: \ent)(\forall j: \ent)(\forall h: \ent)((enRango(t, i)  $\land$ enRango(t, j) $\land$ enRango(t[i], h)$\land$ 
	i \neq j)  $\implicaLuego$ (t[i][h] \neq t[j][h])\\
        }	
	...
	$\implicaLuego$ (s = Concat(Concat(subseq(s[i], j, j+3), subseq(s[i+1], j, j+3)), subseq(s[i+2], j, j+3))$\land$\\
	noHayRepetidos(s))\\
        }
    ...
    \post
		...
		result = esSub(t_{0},t_{1})
				...
				\pred{esSub}{t_{0}: \matriz{\ent},t_{1}: \matriz{\ent}}{\\
					...
					enRengo(t_{0},i) $\yLuego$ length(t_{0}[i]) = length(t_{1}[i]) $\land$ \\
					enRengo(t_{0}[i],j) $\implicaLuego$ t_{0}[i][f] \neq 0 $\land$ t_{0}[i][f] = t_{1}[i][f]
...
12. \begin{proc}{sudoku\_tieneSolucion}{\textbf{\In t}: \matriz{\ent}, \textbf{\Out tienesolucion}: \bool}{}{}
		\pre{tableroValido(t)}
    \post{
		\textbf{tienesolucion} = (solucion(t) \neq t)
		}
		\aux{solucion}{t: \matriz{\ent}}{\matriz{\ent}}
			{\\
				(\exists x: \matriz{\ent})
...
13. \begin{proc}{sudoku\_resolver}{\textbf{inout t}: \matriz{\ent}, \textbf{\Out tienesolucion}: \bool}{}{}
    ...
    \post
		{
		\textbf{tienesolucion} = (esTableroValido(t) $\land$ (solucion(t_{0} ) \neq t_{0})) $\land$ t = solucion(t_{0})
		}
